...
class Keyoku:
    """Synchronous client for Keyoku API.
    ...
    Example:
        ...
        # Store a memory
        job = client.remember("User prefers dark mode")
        job.wait()
    ...
    def remember(
        ...
        *,
        ...
        Returns:
            JobHandle that can be used to wait for completion

        Example:
            ```python
            job = client.remember("User likes pizza")
            result = job.wait()  # Wait for processing
            ```
        """
        ...
        response = self.request("POST", "/v1/memories", json=data)
        return JobHandle(self, response["job_id"])
    ...
    def search(
        ...
        *,
        ...
        response = self.request("POST", "/v1/memories/search", json=data)
...
class JobHandle:
    ...
    def __init__(self, client: Keyoku, job_id: str):
        self.client = client
        self.job_id = job_id

    def get(self) -> Job:
        """Get current job status."""
        response = self.client.request("GET", f"/v1/jobs/{self.job_id}")
        return Job(**response)
